...
export default function ClaimStandardPricing() {
  ...
  useEffect(() => {
    const getPrice = async () => {
      try {
        const r = await fetch(`${baseUrl}/admin/enterprisePrice`);
        const data = await r.json();
        console.log("Price API →", data);
        setPrice(data.enterprisePrice);
      } catch (e) {
        console.error(e);
  ...
  const handlePayment = async () => {
    ...
    try {
      const r = await fetch(`${baseUrl}/claim/pay/${busIdNum}/${claimNum}`, {
        method: "POST",
